...
RSpec.describe 'Movie by year' do
  describe 'As a user' do
    # it "can get a list of movies by year in ascending order by default" do
    #   50.times { create(:movie, release_date: Time.now.utc + 1.days) }
    #   create(:movie, release_date: Time.now.utc - 30.days)
    #   create(:movie, release_date: Time.now.utc - 20.days)
    #
    #   params = {
    #     "release_date" => 2021,
    #     "page" => 1
    ...
    it "can get a list of movies in descending order" do
      50.times { create(:movie, release_date: Time.now.utc - 30.days) }
      create(:movie, release_date: Time.now.utc + 2.days)
      create(:movie, release_date: Time.now.utc + 1.days)
      ...
      first_movie_date = movies_json[:data][0][:attributes][:release_date]
      second_movie_date = movies_json[:data][1][:attributes][:release_date]
      lesser_date = second_movie_date < first_movie_date

      expect(lesser_date).to eq(true)
    end
  end
end
